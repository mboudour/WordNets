...
#     z=PrettyTable(['Vertices','Position','RelPosition','Betweenness'])
    ...
    workbook.close()
def aNala_pandas(fgraphs,traj_list,traj_counter,filf,plot_first_mode=True):
...
##        worksheet.write('X1','Weight')
    ...
    all_nodes,fOriginator, fTransmitter,fTerminator, fBlackHole,fInvolvement,lOriginator,\
    lTransmitter,lTerminator,lBlackHole, lInvolvement,Position,RelPosition ,bet,llength\
    =trajectoriesProp(fgraphs,traj_list,traj_counter)
    pOriginator=dict()
    pTransmitter =dict()
    pTerminator=dict()
    pBlackHole  =dict()
...
def trajectoriesProp(fgraphs,ldicc,traj):
    '''
    ...
    # print type(ldicc),ldicc.keys()
    for key,vallu in ldicc.items():
        print key,vallu
        # print vall
        F=fgraphs[key]
        # print nx.info(F),'mmmm'
        # print F.nodes()
        # print F.edges()
        # fig=plt.figure()
        ...
        for vall in vallu:
            # print vall,'vall'
            # uup=nx.all_simple_paths(F,vall[1],vall[1])
            # print len(list(uup))
            try:
                uup=nx.all_simple_paths(F,vall[0],vall[1])
                ...
                # print kk,'in prop'
                    ...
                    if lldic[0]==lldic[-1]:
                        ...
                        if lldic[0] not in lisBl:
                            lisBl[lldic[0]]=array('h')#=[len(lldic[0])]
                        # else:
...
def creatTestGraph_pandas(start,end,characters,character,quak=600):
    ...
    # ndls=[list(i) for i in ndlss]
    return G#,ndls
def creatTestGraph_pandas_bips(start,end,characters,character,pols,subj,quak=600):
    import datetime
    pold={}
    subjd={}
    # G=nx.davis_southern_women_graph()
    ...
    for i,v in enumerate(characters):
        ...
        if character in v:
            if character == v[0]:
                ...
                ed=v[0]
            G.add_edge(ed,character,date_start=int(start[i]),date_end=int(end[i]))
            G.add_node(ed,attribute='director',bipartite=0)
...
def main_work_search_name(G,ndll,timetoadd,figi,search_name,davisw=False,verb=False,plot_first_mode=True,no_anala=True):
    pdfs={}
    # import pdb; pdb.set_trace()
    ...
    if isinstance(ndll[0],list):
        ...
        for ndls in ndll:
            ...
            for u_node in intervals:
                ...
                # print u_node
                # print start_pa,end_pa,'start end--------------------------'
                for i in start_pa:
                    starters.append(i[1])
                    trajjj=set()
                    # print 'Trajectories of %s starting with %s' %(u_node, i[1])
                    if davisw:
                        ...
                        # print sorted(trajjj, key=lambda traj: traj[0])
                        nw=[]
                        for igi in sorted(trajjj, key=lambda traj: traj[0]):
                            ii=igi[1]
                            # print ii
                            nw.append(ii)
                        # print nw
                        ...
                        # if ndll == ['Thrasymachus']:
                            # pdb.set_trace()
                        for j in end_pa:
                            traj_list[u_node].append([i[1],j[1]])
                            nw_traj[u_node].append([i[1],j[1]])
                            # print nx.info(F),'Inn=============================================='
            ...
            # aNala(traj_list,traj_counter_C,outfile_name_xls)
            if no_anala:
                pdfs[str(ndls)]=aNala_pandas(fgraphs,traj_list,traj_counter_C,outfile_name_xls,plot_first_mode=plot_first_mode)
            else:
                pdfs=None
            if verb:
                print ndls,'done anala'
        ...
        for u_node in intervals:
            ...
            # print u_node
            print start_pa,end_pa,'start,end pa'
            # pdb.set_trace()
        ...
        outfile_name_xls = os.path.join('%s' % filedir,filf)
        # aNala(traj_list,traj_counter_C,outfile_name_xls)
        pdfs=aNala_pandas(fgraphs,traj_list,traj_counter_C,outfile_name_xls)
